...
<head>
    ...
    <title>ACUARIO VALPARAÍSO POS</title>
    <!-- React y Babel para ejecución directa en el navegador -->
...
<body>
    ...
    <script type="text/babel">
        ...
        // Componente de Iconos SVG manuales para evitar errores de librerías externas
        ...
        const INITIAL_PRODUCTS = [
            ...
            { id: 5, name: "Peluche Tiburón Blanco XL", price: 12000, category: "Zoovenir", color: "bg-blue-50", icon: "Gift" },
            ...
            { id: 14, name: "Helado Tritón", price: 2500, category: "Comida", color: "bg-teal-50", icon: "Utensils" },
        ];
        ...
        const App = () => {
            ...
            const handleCreateTour = () => {
                ...
                const msg = `🌊 *ACUARIO VALPARAÍSO*: Nuevo recorrido programado a las ${tourTime} con ${selectedGuide}.`;
                ...
                <div className="flex h-screen overflow-hidden bg-slate-50 text-slate-900 font-sans">
                    ...
                    <aside className="w-20 lg:w-64 bg-[#001d3d] text-white flex flex-col shrink-0 border-r border-blue-900 shadow-xl z-20">
                        ...
                        <div className="p-4 lg:p-6 mt-auto border-t border-blue-900/50 flex items-center gap-3">
                            ...
                            <span className="text-[10px] font-black uppercase hidden lg:block text-sky-200 uppercase">Puerto Valparaíso</span>
                        </div>
                    </aside>
                    ...
                    <main className="flex-1 flex flex-col overflow-hidden">
                        <header className="h-16 bg-white border-b flex items-center justify-between px-6 shrink-0 shadow-sm z-10">
                            ...
                            <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest italic hidden sm:block">Acuario Valparaíso POS v2.0</div>
                        </header>
                        ...
                        <div className="flex-1 overflow-hidden">
                            {currentView === 'pos' && (
                                <div className="flex h-full flex-col lg:flex-row animate-fade">
                                    ...
                                    {/* Carrito Lateral */}
                                    <div className="w-full lg:w-[420px] bg-white border-l flex flex-col shadow-2xl shrink-0">
                                        <div className="p-5 border-b flex justify-between items-center bg-sky-50/30">
                                            ...
                                            {cart.map(item => (
                                                <div key={item.id} className="bg-white p-4 rounded-2xl border flex items-center gap-3 shadow-sm hover:border-sky-100 transition-all animate-fade">
                                                    ...
                                                    <div className="flex items-center bg-slate-100 rounded-xl p-1 border">
                                                        ...
                                                        <span className="w-7 text-center text-[11px] font-black text-slate-700">{item.quantity}</span>
                                        ...
                                        <div className="p-8 bg-[#001d3d] text-white rounded-t-[3rem] shadow-2xl">
                                            <div className="flex justify-between items-center mb-6 px-2">
                                                ...
                                                <span className="text-4xl font-black tracking-tighter text-white">{formatMoney(total)}</span>
                                            </div>
                            ...
                            {currentView === 'tours' && (
                                <div className="flex-1 p-6 lg:p-10 overflow-y-auto bg-sky-50/20 custom-scrollbar animate-fade">
                                    <div className="max-w-4xl mx-auto space-y-8">
                                        <h2 className="text-3xl font-black text-slate-900 uppercase tracking-tighter">Gestión de Recorridos</h2>
                                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                                            <div className="bg-white p-8 rounded-[3rem] border shadow-xl border-blue-50">
                                                <label className="block text-[10px] font-black text-slate-400 uppercase mb-6 text-center tracking-widest leading-none">Seleccionar Guía Encargado</label>
                                                <div className="grid grid-cols-1 gap-2 mb-8">
                                                    {GUIDES.map(g => (
                                                        <button key={g} onClick={() => setSelectedGuide(g)} className={`p-4 rounded-2xl text-left font-bold text-sm border-2 transition-all ${selectedGuide === g ? 'bg-sky-50 border-sky-500 text-sky-700 shadow-md scale-[1.02]' : 'bg-slate-50 border-transparent text-slate-400 hover:bg-slate-100'}`}>{g}</button>
                                                    ))}
                                                </div>
                                                    ...
                                                    {tourHistory.map(t => (
                                                        <div key={t.id} className="bg-white p-6 rounded-[2.5rem] border flex items-center justify-between shadow-md border-sky-50 animate-fade">
                                                            ...
                                                            <div className="text-right">
                                                                ...
                                                                <p className="text-[9px] font-black text-sky-400 uppercase tracking-widest italic leading-none tracking-widest">En preparación</p>
                                                            </div>
                                                        </div>
                                                    ))}
                            ...
                            {currentView === 'history' && (
                                <div className="flex-1 p-6 lg:p-10 overflow-y-auto custom-scrollbar animate-fade">
                                    <div className="max-w-3xl mx-auto space-y-4">
                                        <h2 className="text-3xl font-black text-slate-900 uppercase tracking-tighter mb-8 text-center lg:text-left">Bitácora de Ventas</h2>
                                        <div className="space-y-3">
                                            {salesHistory.map(s => (
                                                <div key={s.id} className="bg-white p-6 rounded-[2.5rem] border flex items-center justify-between shadow-sm hover:border-sky-200 transition-all group animate-fade">
                                                    <div className="flex items-center gap-6">
                                                        ...
                                                        <div>
                                                            ...
                                                            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest leading-none">{s.time} • {s.method.toUpperCase()}</p>
                                                        </div>
                                                    </div>
                                                    <p className="text-2xl font-black text-sky-800 tabular-nums tracking-tight">{formatMoney(s.total)}</p>
                                                </div>
                                            ))}
                                            {salesHistory.length === 0 && <div className="p-20 text-center text-slate-300 font-black uppercase italic text-sm">No hay ventas registradas hoy</div>}
                                        </div>
                                    </div>
                                </div>
                            )}
                                            ...
                                            <table className="w-full text-left">
                                                <thead className="bg-slate-50 border-b font-black text-[10px] text-slate-400 uppercase tracking-widest">
                                                    <tr>
                                                        ...
                                                        <th className="p-8 text-right">Recaudación</th>
                                                    </tr>
                                                </thead>
                        ...
                        <div className="fixed inset-0 bg-[#001d3d]/95 backdrop-blur-xl z-[100] flex items-center justify-center p-4">
                            <div className="bg-white w-full max-w-sm rounded-[4rem] shadow-2xl overflow-hidden animate-fade">
                                <div className="p-10 text-center bg-[#001d3d] text-white relative">
                                    ...
                                    <h3 className="text-6xl font-black tracking-tighter text-white">{formatMoney(total)}</h3>
                                </div>
                                <div className="p-10 space-y-8 text-center">
                                    <div className="flex gap-3">
                                        ...
                                        <button onClick={() => setPaymentMethod('debito')} className={`flex-1 p-5 rounded-3xl border-2 transition-all font-black uppercase text-[10px] ${paymentMethod === 'debito' ? 'bg-blue-50 border-blue-500 text-blue-700 shadow-md' : 'text-slate-300 border-transparent bg-slate-50'}`}>Débito</button>
                                    </div>
                                    ...
                                    <div className="space-y-4 pt-4">
                                        ...
                                        <button onClick={() => { setIsCheckoutOpen(false); setCashReceived(''); }} className="w-full text-slate-400 font-black uppercase text-[10px] text-center tracking-[0.2em] hover:text-slate-600 transition-colors uppercase leading-none">Cancelar Operación</button>
                                    </div>
                                </div>
                            </div>
                        </div>
